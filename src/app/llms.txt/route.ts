...
export async function GET() {
  const baseUrl = (process.env.NEXT_PUBLIC_SITE_URL || 'https://dead-as-disco.wiki').replace(/\/$/, '')
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Dead as Disco Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a comprehensive game wiki covering Idols, songs, builds, My Music, Infinite Disco, and roadmap updates.`)
  lines.push('')
